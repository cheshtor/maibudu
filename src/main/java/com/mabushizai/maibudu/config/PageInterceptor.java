...
public class PageInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        PageModel pageModel = getPageModel(parameterObject);
        if (null == pageModel) {
            return invocation.proceed();
        }
        long total = getTotal(invocation, metaObject, boundSql);
        pageModel.setTotalCount(total);
        return rewriteSql(invocation, metaObject, pageModel);
    }
    ...
    /**
     * 从传入 DAO 层方法的参数中找到携带分页参数的 {@link PageModel} 对象
     *
     * @param parameterObject 传入 DAO 层方法的参数
     * @return 携带分页参数的 {@link PageModel} 对象
     */
    private PageModel getPageModel(Object parameterObject) {
        ...
        } else if (parameterObject instanceof PageModel) {
            return (PageModel) parameterObject;
        }
        return null;
    }
     ...
     * @param invocation 代理执行上下文
     * @param metaObject StatementHandler 包装器
     * @param pageModel     {@link PageModel} 分页模型
     * @return 重写后的 SQL
     * @throws Exception
     */
    private Object rewriteSql(Invocation invocation, MetaObject metaObject, PageModel pageModel) throws Exception {
        String sql = getSql(metaObject);
        ...
        preparedStatement.setLong(parameterCount - 1, (pageModel.getPageNo() - 1) * pageModel.getPageSize());
        preparedStatement.setLong(parameterCount, pageModel.getPageSize());
